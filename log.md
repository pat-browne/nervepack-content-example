...
> **Template.** `log.md` is an **append-only** chronological audit of everything that
> lands in your overlay — source ingests, wiki edits, skill adds, setup changes. Entry
> format is `## [YYYY-MM-DD] <type> | <summary>` so `grep "^## \[" log.md` parses it.
